...
import { Menu, X, CreditCard, BookOpen, GraduationCap, Calendar, Megaphone, Clock, Newspaper, Phone, LogIn, LayoutDashboard, User, LogOut, Shield } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 border-b border-cyan-500/20 bg-[#0a0f1a]/95 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between min-h-[4rem] sm:min-h-[4.5rem] py-2">
          ...
          <nav className="hidden lg:flex items-center gap-0.5 flex-wrap justify-end max-w-3xl">
            {NAV.filter((n) => n.to !== '/pagar-cuota' || isStudent).map(({ to, label, icon: Icon }) => (
              <NavLink
                key={to}
                to={to}
                end={to === '/'}
            ...
            {user ? (
              <>
                {isAdmin && (
                  <Link
                    to="/admin"
                    className="flex items-center gap-1.5 px-3 py-2.5 rounded-lg text-sm font-medium text-amber-400 hover:bg-amber-500/10 whitespace-nowrap"
                  >
        ...
        {menuOpen && (
          <div className="lg:hidden border-t border-cyan-500/20 bg-[#0d1321] px-4 py-4 flex flex-col gap-1">
            ...
            {user ? (
              <>
                {isAdmin && <Link to="/admin" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-amber-400">Admin</Link>}
      ...
      <footer className="border-t border-cyan-500/20 bg-[#070b12] py-10 px-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <nav className="flex flex-wrap justify-center sm:justify-end gap-x-6 gap-y-1 text-sm text-slate-400">
            {isStudent && <Link to="/pagar-cuota" className="hover:text-cyan-400 transition-colors">Pagar cuota</Link>}
